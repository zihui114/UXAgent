    ...
    def analyze_test_results(
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                max_tokens=12000,  # 增加到12000讓LLM有足夠空間輸出詳細分析
...
{{
  ...
  "recommendations": [
    {{
      ...
      "specific_actions": [
        "在產品頁面頂部新增『🏅 官方認證資訊』區塊，包含：衛福部健字號（例如：衛署健食字第 AXXXXXX 號）",
        "提供『查看認證證書 (PDF)』下載連結和『🔗 衛福部 TFDA 資料庫查詢』外部連結",
        "新增『SGS 檢驗報告』區塊，提供：胺基酸組成分析、普林含量檢測（XX mg/100ml）、微生物檢驗報告下載",
        "在產品標題旁加入認證徽章圖示（如綠色勾勾 + 『衛福部認證』文字）",
        "使用顯眼的設計（高對比色、較大字體）確保使用者第一眼就能看到"
      ],
